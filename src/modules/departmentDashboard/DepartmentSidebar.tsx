...
import { useNavigate, useLocation, useParams } from "react-router-dom"; // Updated import
...
const DepartmentSidebar: React.FC<DepartmentSidebarProps> = ({ open, setOpen }) => {
  ...
  const departmentPath = `/department-dashboard/${departmentName}`;

  const menuItems = [
    { text: "Dashboard", icon: <DashboardIcon />, path: `${departmentPath}` },
    { text: "Member", icon: <GroupIcon />, path: `${departmentPath}/members` },
    { text: "Reports", icon: <BarChartIcon />, path: `${departmentPath}/reports` },
  ];

  const bottomMenuItems = [
    { text: "Settings", icon: <SettingsIcon />, path: `${departmentPath}/settings` },
          ...
          <List>
            {menuItems.map((item, index) => (
              <ListItemButton
                ...
                sx={{
                  padding: "6px 6px",
                  bgcolor: location.pathname === item.path ? "#e4bfa7" : "transparent", // Highlight active page
